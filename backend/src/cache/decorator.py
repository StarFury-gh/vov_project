...
# получение последней версии кеша
async def _get_cache_version(redis_client: Redis | None = None) -> int:
    """Возвращает последнюю версию кеша"""
    if redis_client is None:
        from cache.client import get_redis
        redis_client = get_redis()

    version = await redis_client.get("app:cache_version")
    if version is None:
        await redis_client.setnx("app:cache_version", 1)
        return 1
    return int(version)
...
def make_cache_key(
    ...
    Формат ключа:  
    -------
    app:version:module:qualname:args_hash
    """
    ...
    return f"app:{cache_version}:{func.__module__}.{func.__qualname__}:{args_hash}"
...
# декоратор инвалидации кеша
def invalidate_cache() -> Callable[[F], F]:
    def decorator(func: F) -> F:
        @functools.wraps(func)
        async def wrapper(*args: Any, **kwargs: Any) -> Any:
            from cache.client import get_redis
            client: Redis = get_redis()
            await client.incr("app:cache_version")
            logger.info(f"Redis SET new cache version")
    ...
    *,
    ...
    def decorator(func: F) -> F:
        ...
        async def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            cache_version = await _get_cache_version(client)
            key = make_cache_key(func, args, kwargs, cache_version)
            ...
            # запись в кеш
            try:
                logger.info("Redis SET new key: %s", key)
